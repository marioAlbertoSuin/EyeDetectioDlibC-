...
HibridMedianfilter::HibridMedianfilter() : inputfilename(0), outputfilename(0)
{
	;
}

HibridMedianfilter::HibridMedianfilter(HibridMedianfilter &src)
{
	if (this != &src)
	{
...
HibridMedianfilter::HibridMedianfilter(std::string inputname, std::string outputname)
{
	inputfilename = inputname;
	outputfilename = outputname;
}
HibridMedianfilter &HibridMedianfilter::operator=(const HibridMedianfilter &src)
{
	if (this != &src)
	{
		inputfilename = src.inputfilename;
...
HibridMedianfilter::~HibridMedianfilter()
{
}

//function apply bubble sort in size of array[arraysize] output the median number
unsigned char HibridMedianfilter::median(unsigned char *src, unsigned int arraysize)
{
...
#pragma omp parallel
	...
	{
#pragma omp for private(j, i, src)
...
void HibridMedianfilter::Set_Input_path(std::string inputname) { inputfilename = inputname; }

void HibridMedianfilter::Set_output_path(std::string outputname) { outputfilename = outputname; }

std::string HibridMedianfilter::Get_Input_path() const { return inputfilename; }

std::string HibridMedianfilter::Get_Output_path() const { return outputfilename; }
...
int HibridMedianfilter::applyfilter(unsigned int k)
